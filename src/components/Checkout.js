...
function Checkout() { 
  ...
  return (
    ...
    <hr></hr>
    <div id="hotelData">
        <div><img id="hotelImg" src={HotelData.hotelImg} alt="hotelImage" /></div>
        <div id="contentsText">
        <div id="Hname"><u><h2>{HotelData.hotelName}</h2></u><p>{HotelData.hotelLocation}</p></div>
        <div id="stars"><h2>{HotelData.hotelStars}</h2></div>
        <div id="start">
          <div><h2>Check In</h2></div>
          <div><h2>{startDate}</h2></div>
        </div>
        <div id="end">
          <div><h2>Check Out</h2></div>
          <div><h2>{endDate}</h2></div>
        </div>
      </div>
    </div>
    <div id="rules">
    ...
    <ul>
      ...
      <li>This property and its staff is implementing rigorous health & safety measures guidelines.</li>
    ...
    <div id="breakup">
      ...
      <div><span>&#8377;</span>{HotelData.hotelPrice}</div>
      <div>Taxes and Services</div>
      <div><span>&#8377;</span>{(0.18*(HotelData.hotelPrice))}</div>
      <div><h5>Total Amount</h5></div>
      <div><span>&#8377;</span>{HotelData.hotelPrice + (0.18*(HotelData.hotelPrice))}</div>
    </div>
    </div>
    </div>
